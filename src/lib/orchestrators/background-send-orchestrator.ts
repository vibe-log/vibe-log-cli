...
export class BackgroundSendOrchestrator {
  async execute(options: SendOptions): Promise<void> {
    ...
    // NOTE: Version checking is now handled in send.ts before routing to this orchestrator
    // The non-blocking update mechanism in send.ts ensures we have the latest version
    // without blocking hook execution. See send.ts:updateInBackground()

    // Spawn detached background process with current version
